...
从 uTools 搜索结果打开“滚动长截图”后，langShot 使用 `mainHide` 保持 uTools 主窗口隐藏并直接进入鼠标框选：
...
- 点击“矩形”，拖拽绘制空心圆角矩形以圈定重点内容。
- 点击“文字”，再点击图片中的位置，输入内容并按 `Enter` 确认；输入时按 `Esc` 取消。
- 使用默认“选择”工具拖动箭头、矩形或文字改变位置；选中箭头后可拖动两个端点调整方向和长度，选中矩形后可拖动四角调整大小，双击文字可重新编辑内容。
