...
@Test func retinaCoordinatesUsePhysicalPixelsAndTopLeftOrigin() {
    ...
    let result = display.pixelRect(for: RectValue(x: 200, y: 700, width: 720, height: 200))
...
@Test func selectionIsClippedToOneDisplay() {
    ...
    #expect(display.pixelRect(for: RectValue(x: 90, y: -20, width: 30, height: 60)) == PixelRect(x: 90, y: 40, width: 10, height: 40))
}
